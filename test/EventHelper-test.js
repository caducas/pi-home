...
/*
	it("3", function() {
		...
		assert(stub3.calledOnce, "method 'sendToServer' from module 'networkCommunicator' wasn't called for condition 'Button01Pressed'");
		assert(stub3.calledWith(testObjectToSend), "method 'sendToServer' from module 'networkCommunicator' wasn't called with correct arguments");
		assert(stub4.calledTwice, "method 'checkCondition' from 'conditionHelper' wasn't called");
		
		networkCommunicator.sendToServer.restore();
		conditionHelper.checkCondition.restore();

		stub3 = sinon.stub(networkCommunicator,"sendToServer");
		stub4 = sinon.stub(conditionHelper, "checkCondition");

		testObjectToSend = {"command" : "event", "listener" : "Button01", "condition" : "Button01Released"};
		stub4.withArgs()
		process.emit("Button01",0);

		assert(stub3.calledOnce, "method 'sendToServer' from module 'networkCommunicator' wasn't called for condition 'Button01Released'");
		assert(stub3.calledWith(testObjectToSend), "method 'sendToServer' from module 'networkCommunicator' wasn't called with correct arguments");
		
		networkCommunicator.sendToServer.restore();

/*
		//check for not execution if second condition is true but first one not true
		var stub4 = sinon.stub(eventHelper,"checkCondition").withArgs(0, {"operator":"=","value":1}).returns(false);
		var stub5 = sinon.stub(eventHelper,"checkCondition").withArgs(0, {"operator":"=","value":0}).returns(true);
		var testObjectToSend = {"command" : "event", "listener" : "Button01", "condition" : "Button01Released"};
		process.emit("Button01",0);

		assert(stub3.calledOnce, "method 'sendToServer' from module 'networkCommunicator' wasn't called");
		assert(stub3.calledWith(testObjectToSend), "method 'sendToServer' from module 'networkCommunicator' wasn't called with correct arguments");
		
		eventHelper.checkCondition.restore();
		networkCommunicator.sendToServer.restore();
	});
		*/
